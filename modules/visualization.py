...
def plot_candlestick(ax, stock_data):
    ...
    """
    logger.debug(f"plot_candlestick 輸入資料: 筆數={len(stock_data)}, 索引範圍={stock_data.index.min()}-{stock_data.index.max()}")
...
def plot_charts(codes: list, prices: pd.DataFrame, ma_period: int = 20) -> str:
    """
    繪製最多 6 支股票的 K 棒圖（2x3 子圖），搭配指定週期的移動平均線
    ...
    ma_color = '#2E86DE' if ma_period >= 20 else '#E67E22'

    logger.debug(f'開始繪製圖表（{ma_label}），股票代碼: {codes}')
    logger.debug(f'價格數據總筆數: {len(prices)}')

    if not prices.empty and 'date' in prices.columns:
        logger.debug(f'價格數據日期範圍: {prices["date"].min()} ~ {prices["date"].max()}')
    ...
    min_data_rows = ma_period  # MA 需要至少 ma_period 筆

    for i, code in enumerate(codes):
        stock_data = prices[prices["code"] == code].sort_values("date").tail(90)

        logger.debug(f'股票 {code}: 原始 {len(prices[prices["code"] == code])} 筆, tail(90) {len(stock_data)} 筆')
        ...
        stock_data[ma_col] = stock_data["close"].rolling(ma_period, min_periods=ma_period).mean()

        ax = axes[i]
        plot_candlestick(ax, stock_data)

        # 繪製移動平均線
        valid_ma = stock_data[stock_data[ma_col].notna()]
        if not valid_ma.empty:
            ma_indices = valid_ma.index - stock_data.index[0]
            ax.plot(ma_indices, valid_ma[ma_col], label=ma_label,
                   linewidth=2, linestyle="--", alpha=0.7, color=ma_color)
        ...
        tick_labels = [date_labels[j] for j in tick_positions]
    ...
    logger.info(f"✅ 圖表已生成（{ma_label}）: {temp_file.name}")
    return temp_file.name


# 向後相容：保留舊函數名作為 alias
def plot_stock_charts(codes: list, prices: pd.DataFrame) -> str:
    """繪製 MA20 K 棒圖（plot_charts 的 alias）"""
    return plot_charts(codes, prices, ma_period=20)


def plot_breakout_charts(codes: list, prices: pd.DataFrame) -> str:
    """繪製 MA10 K 棒圖（plot_charts 的 alias）"""
    return plot_charts(codes, prices, ma_period=10)